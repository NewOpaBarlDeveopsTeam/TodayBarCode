...
//             taxvalue for no
            ...
            console.log(catlength)
              var itemgst = 0;
              $scope.gst = 0;
             for(m=0;m<catlength;m++)
        {
            
            
           // console.log($scope.catitem1[n].total);
      $scope.subtol += parseInt($scope.catitem1[m].total);
            console.log($scope.subtol)
             //console.log($scope.catitem1[n].count);
            $scope.totquant += parseInt($scope.catitem1[m].count);
            //console.log($scope.totquant)
           // $scope.serchar=$scope.subtol*0.1;
            //console.log($scope.serchar)
...
//            //console.log($scope.total)
             ...
             console.log($scope.catitem1)
            console.log($scope.catitem1[m].totalgst)
           itemgst = parseInt($scope.catitem1[m].totalgst)
            console.log(itemgst)
             $scope.gst  += itemgst;
         ...
         console.log(restbill) 
           ...
           console.log(catitem1.length)
            ...
            console.log(catitem1[i].total)
                 ...
                 console.log(hidtot)
var poscartt =mobile+","+catitem1[i].name+","+catitem1[i].section+","+catitem1[i].count+","+catitem1[i].total+","+total+","+totquant+","+serchar+","+cgst+","+sgst+","+gst+","+subtol+","+myVar+","+i+","+catitem1[i].selected+","+rschange+","+change+","+sesdate+","+session+","+restbill+","+catitem1[i].positemtype+","+hidtot+","+catitem1[i].totalgst;
...
//        }) 
         ...
         $scope.closeday=function(sesdate,session){
             ...
             $http.post('/sesdatee'+binddate11).success(function(response){
              ...
              window.sessionStorage.setItem("sesDate",sesdate);
             window.sessionStorage.setItem("sessionInDay",0);
                               ...
                               // console.log("arracon5")
                                // console.log($scope.arrcon5)
                                     ...
                                     for(h=0;h<=billcarlength-1;h++){
                                       // alert("2237"+h)
                                         console.log(h)
                                     console.log($scope.billcar[j].tax)
                                         if(h!=billcarlength-1){
                                     console.log($scope.billcar[j])
                                        console.log($scope.billcar[j].tax[h])
//                                         console.log($scope.billcar[j].tax[j].Rate)
                                       var tgst = parseInt($scope.billcar[j].tax[j].Rate);
                                        console.log(tgst)
                                        $scope.tgst+=tgst;
                                        //console.log($scope.arrcon5[j].itemtota)
                                         //console.log($scope.arrcon5[h].itemtota)
                                         hidgst = parseFloat($scope.arrcon5[j].itemtota*$scope.tgst/100);
//                                        console.log(total)
                                         console.log(billlength)
                                       
                                         
                                        hidrate= parseFloat($scope.arrcon5[j].itemtota-hidgst);
                                         //console.log(hidrate)
                                       // alert(hidrate)
                                          obj1["hidgst"] = hidgst;
                                ...
                                for(a=0;a<$scope.billcar.length;a++){
                                    ...
                                    for(b=0;b<$scope.billcar.length;b++){
                                        if($scope.billcar[a].itemtype == $scope.billcar[b].itemtype){
                                            if(duplicate.indexOf($scope.billcar[b].name)==-1){
                                                var object = {};
                                                 object["itemtype"] = $scope.billcar[b].itemtype;
                                                 object["name"] = $scope.billcar[b].name;
                                                 object["quantity"] = $scope.billcar[b].quantity;
                                               
                                                 duplicate.push($scope.billcar[b].name);
                                                requriedobj.push(object)
                                                console.log(requriedobj)
                                                $scope.kottbill = requriedobj;
                                                
//                                                var objjjj = {};
//objjjj["itemtype"].["name"] = requriedobj.itemtype;
//objjjj["itemtype"].["quantity"] = requriedobj.name;
//nietos.push(objjjj);
//       console.log(nietos)                                       

                                               
                                            
//                  $scope.kottbill = duplicate;
//                                                console.log($scope.kottbill)
                                                
                                           console.log("aaaaaaaa"+$scope.billcar[b].itemtype)
                                            }
                                        }
//                                        
                                    }
